...
- Added the admin Webhooks UI for endpoint setup, verification secret storage, routing visibility, event inspection, delivery errors, and retrying failed deliveries.
...
- Updated the Telegram webhook experiment to mention `@RolinoBot` directly with a fetch-and-process action for the PostRiderAI message.
- Hardened endpoint secret resolution so malformed signatures are rejected before secret work and undecryptable endpoint secrets are skipped during fallback scans.
- Fixed the Webhooks setup card to show and copy the absolute production ingress URL instead of a relative `/api/...` path.
- Switched the PostRider setup flow to the standard sender-generated secret model: Control Center now displays a receiver URL with the endpoint id in the query string and stores the verification secret generated by PostRider instead of generating/regenerating its own signing secret.
- Documented `X-PostRider-Signature` and `X-PostRider-Timestamp` as the canonical PostRider webhook headers; generic webhook headers remain compatibility fallbacks only.
- Accepted signed PostRider `webhook.test` events as connectivity checks that refresh endpoint activity without validating them as `message.received` payloads or creating agent events.
- Logged signed PostRider `webhook.test` checks in the Webhooks Events tab as ignored events, and moved provider display copy into source configuration so the Webhooks page is ready for additional sources.
...
- Auto-refreshed the admin Webhooks events list via a 30s `router.refresh()` poll that runs only while the Events tab is open and the browser tab is focused (plus an immediate refresh on entering the tab or regaining focus), so new webhook events appear without a manual page refresh while keeping server/DB load minimal.
